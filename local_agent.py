...
class ReactAgent:
    """
    A minimalist agent that uses Ollama Python client to play Pokemon.
    
    This agent reads the game log file for context and uses a local LLM via Ollama 
    to decide what commands to send to the game.
    """
    
    def __init__(self, model_name="deepseek-r1"):
        """
        Initialize the ReactAgent with Ollama Python client.
        
        Args:
            model_name (str, optional): Which Ollama model to use. Defaults to "deepseek-r1".
        """
        ...
        try:
            ...
            if 'models' in models:
                ...
                if self.model_name not in available_models:
                    logger.warning(f"Model {self.model_name} not found in Ollama. Verify it's installed.")
            else:
    ...
    async def chat_with_ollama(self, prompt):
        """
        Chat with Ollama using Python client and get the response.
        ...
        try:
            ...
            # Log that we're sending a request to Ollama
            logger.info("Sending request to Ollama...")
            logger.info("----- OLLAMA REQUEST START -----")
            
            # Use the chat method from ollama library but handle streaming correctly
            # Use non-streaming first as a safer approach
            ...
            logger.info("----- OLLAMA RESPONSE END -----")
            ...
            # Keep history manageable - retain last 5 exchanges
    ...
    async def get_command(self):
        ...
        try:
            ...
            logger.info("Asking Ollama LLM for next command...")
            response_text = await self.chat_with_ollama(prompt)
            ...
            if not command:
                logger.warning("No command found in LLM's response, defaulting to 'a'")
                command = "a"
            
            logger.info(f"Final command extracted: {command}")
            return command
